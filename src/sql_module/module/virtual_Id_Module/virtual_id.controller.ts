...
class virtualIdSqlController {
  static async genarateVirtualId(
    request: Request,
    response: Response,
    ...
    try {
      ...
      if (!username) {
        return response
          .status(400)
          .send(new HttpException(400, "Username and password are required"));
      }
      virtualIdSqlSqlService.genarateId(username, (err: any, result: any) => {
        if (err) {
          response
            .status(400)
            .send(new HttpException(400, "Something went wrong"));
        } else {
          response
            .status(200)
            .send(new HttpResponse(null, result, "Virtual_id generated", null));
        }
      });
    } catch (err) {
