            ...
            if (error) {
                ...
                await AdaptiveLearningServices.addSchoolUdise(schoolData, (err: any, result: any) => {
                    if (err) {
                        ...
                        response.status(200).send(new HttpResponse(null, result, "School data added", null, (request as any).version));
                    }
                });
            }
        }
            ...
            if (error) {
                ...
                await AdaptiveLearningServices.validateUdise(udiseCode, (err: any, result: any) => {
                    if (err) {
                        ...
                        response.status(200).send(new HttpResponse("GetSchoolData", result, "School Data returned", null, (request as any).version));
                    }
                });
            }
        } catch (err) {
            ...
            if (error) {
                ...
                await AdaptiveLearningServices.deleteUdise(udiseCode, (err: any, result: any) => {
                    if (err) {
                        ...
                        response.status(200).send(new HttpResponse("DeleteSchoolData", result, "udise code deleted", null, (request as any).version));
                    }
                });
            }
        } catch (err) {
        ...
        try {
            await AdaptiveLearningServices.getAllUdeise((err: any, result: any) => {
                if (err) {
                    ...
                    response.status(200).send(new HttpResponse("getAllUdeise", result, "School Data returned", null, (request as any).version));
                }
            });
        } catch (err) {
            response.status(400).send(new HttpException(400, "Something went wrong"));
